document.addEventListener("DOMContentLoaded", function() {
    ...
    formatElement(orderPrice);
    formatElement(deliveryCost);
    formatElement(finalTotalPrice);
    // updateValue 함수 정의
    ...
    function updatePrices(inputElement) {
        ...
        const deliveryCost = unformat((document.querySelector("#deliveryCost").textContent)); // 가격 가져오기
        const orderPriceDisplay = document.querySelector("#orderPrice"); // 주문 가격 표시 요소
        const finalTotalPriceDisplay = document.querySelector("#finalTotalPrice"); // 최종 가격 표시 요소
        let orderPrice = unformat(orderPriceDisplay.textContent);

        // 수량 변화 시 총 가격 업데이트
        orderPrice -= unformat(totalPriceDisplay.textContent);
        ...
        orderPriceDisplay.textContent = format(orderPrice); // 주문 가격 화면에 표시
        finalTotalPriceDisplay.textContent = format(orderPrice + deliveryCost); // 최종 가격을 화면에 표시
    }
